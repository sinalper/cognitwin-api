...
// CRITICAL: All questions must be in FIRST PERSON — "Siz ne yaparsınız?" style.
// NEVER ask what a third party would do. Always address the participant directly as "siz/sen".

const SYS_INIT = `You are conducting a FALSIFICATIONIST cognitive profiling assessment. Understand HOW a participant thinks by generating hypotheses about their reasoning patterns and systematically trying to DISPROVE them.

Use the SPECIFIC assigned topic. FALSIFICATION (Popperian): try to FALSIFY hypotheses, not confirm. Surviving hypothesis wins.

CRITICAL QUESTION FORMAT RULES:
- ALL questions must be written in FIRST PERSON perspective addressing the participant directly
- Use "siz", "sizin", "sizi", "sizde" — always ask what THE PARTICIPANT would do
- NEVER ask what "bir kişi", "biri", "bir arkadaşınız" or any third party would do
- Correct: "Böyle bir durumda ne yaparsınız?" / "Hangisini tercih edersiniz?"
- Wrong: "Bir kişi bu durumda ne yapmalıdır?" / "Arkadaşınız ne yapardı?"
- The scenario can describe a situation involving others, but the CHOICE must be about what the participant themselves would do

TASK:
1. Generate binary A/B scenario in Turkish (realistic, everyday, first-person direct address)
2. Generate ALL plausible hypotheses — each describes a reasoning PATTERN with testable PREDICTION (A or B)

JSON ONLY:
{"category":"...","subtopic":"...","question":{"scenario":"Turkish — must address participant directly","option_a":"Turkish","option_b":"Turkish"},"hypotheses":[{"id":"H1","label":"English label","description":"English reasoning pattern","prediction":"A or B"},{"id":"H2","label":"...","description":"...","prediction":"A or B"}]}`;

const SYS_UPDATE = `FALSIFICATIONIST profiling continuation.

CRITICAL: All new questions must use FIRST PERSON — address the participant directly ("siz/sizin/hangisini tercih edersiniz"). Never ask about third parties.

STRICT FALSIFICATION RULES:
1. Each alive hypothesis: does answer+explanation CONTRADICT its prediction?
2. Verdicts: "clear_contradiction" (1 strike = immediate elimination) | "partial_contradiction" (soft strike, 2nd = elimination) | "no_contradiction" (survives, no strengthening)
3. After: 1 alive → sole_survivor:true | 0 alive → all_eliminated:true (generate up to 3 new) | 2+ → next falsification question
4. Question design: pick alive hypothesis, create scenario where it predicts clearly, test it. FIRST PERSON format always.

JSON ONLY:
{"hypothesis_updates":[{"id":"H1","label":"...","status":"alive|eliminated","verdict":"clear_contradiction|partial_contradiction|no_contradiction","strikes_before":0,"strikes_after":0,"evidence":"..."}],"new_hypotheses":[],"alive_count":2,"sole_survivor":false,"all_eliminated":false,"reasoning":"...","next_question":{"scenario":"Turkish first-person or null","option_a":"Turkish","option_b":"Turkish","target_hypothesis":"...","falsification_logic":"..."}}`;

const SYS_FINAL = `Complete FALSIFICATIONIST profiling.

CRITICAL: All 3 test questions must use FIRST PERSON — address participant directly ("siz/sizin"). Never ask about third parties.

TASK: State winning hypothesis, list eliminations, summarize reasoning (2-3 English sentences), generate 3 TEST questions (same subtopic, different aspects, Turkish first-person A/B, predictions with confidence 0.0-1.0).

JSON ONLY:
{"participant_profile":"English 2-3 sentences","winning_hypothesis":{"id":"...","label":"...","description":"...","strikes":0},"eliminated_hypotheses":[{"id":"...","label":"...","eliminated_at_round":1,"total_strikes":1,"kill_shot":"clear_contradiction|second_strike","evidence":"..."}],"test_questions":[{"id":"T1","scenario":"Turkish first-person","option_a":"Turkish","option_b":"Turkish","prediction":"A or B","confidence":0.75,"prediction_reasoning_tr":"Türkçe: bu tahminin gerekçesi — katılımcının hangi akıl yürütme örüntüsüne dayanarak hangi seçeneği seçeceği"},{"id":"T2","scenario":"Turkish","option_a":"Turkish","option_b":"Turkish","prediction":"A or B","confidence":0.7,"prediction_reasoning_tr":"Türkçe gerekçe"},{"id":"T3","scenario":"Turkish","option_a":"Turkish","option_b":"Turkish","prediction":"A or B","confidence":0.7,"prediction_reasoning_tr":"Türkçe gerekçe"}]}`;

const SYS_REVISE = `You predicted a participant's answer but were WRONG. The participant explained why. Analyze and revise the hypothesis if needed.

TASK:
1. Analyze each misprediction + participant's Turkish explanation
2. Determine if winning hypothesis needs revision
3. If yes: provide revised hypothesis accounting for new evidence
4. If no: explain why hypothesis still holds

JSON ONLY:
{"needs_revision":true,"original_hypothesis":{"label":"English","description":"English"},"revised_hypothesis":{"label":"English","description":"English — must be more nuanced than original"},"revision_reasoning_tr":"Türkçe: misprediction'ların ortaya koyduğu şey ve hipotezin nasıl güncellendiği","misprediction_analysis":[{"test_id":"T1","predicted":"A","actual":"B","participant_explanation":"...","analysis_tr":"Türkçe: bu tahmin neden tutmadı"}]}`;
...
export default function Home(){
  ...
  const [currentFeedbackQ,setCurrentFeedbackQ]=useState(null);
  ...
  const resetCycle=()=>{setTopic(null);setCurrentQ(null);setAnswer(null);setReason("");setProfHist([]);setProfRound(0);setHypotheses([]);setTestQs([]);setTestIdx(0);setTestAns({});setTestFeedback({});setShowFeedback(false);setFeedbackText("");setCurrentFeedbackQ(null);setProfile(null);setRevisedHyp(null);setElimLog([]);fullLog.current=[];};

  const startCycle=async()=>{
    setLoading(true);setLoadMsg("Konu "+cycle+"/"+TOTAL_CYCLES+" seçiliyor...");setError(null);
    try{const ch=await getRandomTopic();const result=await callAPI(SYS_INIT,"ASSIGNED TOPIC:\nID:"+ch.id+"\nCategory:"+ch.cat+"\nSubtopic:"+ch.sub+"\n\nGenerate first question and hypotheses. Remember: first-person format only.");
  ...
  const submitProf=async()=>{
    ...
    try{
      ...
      p+="\nROUND "+profRound+": Evaluate. Generate next question in first-person Turkish if needed.";
  ...
  const genTest=async(hist,hyps)=>{
    ...
    try{
      ...
      p+="\nSURVIVOR:"+winner.id+" ["+winner.label+"]\nGenerate profile, eliminations, 3 first-person test questions with Turkish prediction reasoning.";
  ...
  const submitTestAnswer=()=>{
    ...
    if(correct){advanceTest();}
    else{setCurrentFeedbackQ(tq);setShowFeedback(true);}
  };

  const submitFeedback=()=>{
    setTestFeedback(prev=>({...prev,[testIdx]:feedbackText}));
    setShowFeedback(false);setFeedbackText("");setCurrentFeedbackQ(null);
    advanceTest();
  };
  ...
  const checkRevision=async()=>{
    ...
    setLoading(true);setLoadMsg("Hipotez değerlendiriliyor...");
    try{
      let p="TOPIC:"+topic.category+">"+topic.subtopic+"\n\n";
      p+="WINNING HYPOTHESIS: "+profile.hypothesis.label+" — "+profile.hypothesis.description+"\n\n";
      p+="MISPREDICTIONS WITH PARTICIPANT EXPLANATIONS:\n";
      misses.forEach(m=>{p+="  "+m.id+": Predicted "+m.prediction+", Actual "+m.actual+"\n  Participant's explanation (Turkish): \""+m.feedback+"\"\n\n";});
      p+="Analyze and revise if needed. Provide Turkish reasoning.";
      const result=await callAPI(SYS_REVISE,p);setRevisedHyp(result);
    }catch(e){/* proceed without revision */}
  ...
  const finishCycle=()=>{
    ...
    const cd2={cycle,topic,profiling_rounds:profHist.length,profiling_history:profHist,
      ...
      elimination_log:elimLog,profile,winning_hypothesis:profile?.hypothesis,
  ...
  const dlCSV=()=>{if(!adminData?.length)return;
    ...
    const rows=[];adminData.forEach(r=>{(r.cycles||[]).forEach(c=>{rows.push([r.pid,r.timestamp,r.demographics?.gender||"",r.demographics?.birth_year||"",
      ...
      c.revised_hypothesis?.needs_revision?"evet":"hayir",
  ...
  // ADMIN
  if(phase==="admin"){
    const ag=adminData?{n:adminData.length,correct:adminData.reduce((s,r)=>s+(r.overall_accuracy?.correct||0),0),total:adminData.length*TOTAL_CYCLES*3}:null;
    return(<div style={{...pg,maxWidth:1200}}><Head><title>Admin</title></Head><div style={cd}>
      ...
      {adminData&&<>
        {ag&&ag.n>0&&<div style={{display:"flex",gap:10,marginBottom:16,flexWrap:"wrap"}}>
          {[["Katılımcı",ag.n,"#1a1a2e"],["Overall",ag.total>0?Math.round(ag.correct/ag.total*100)+"%":"—","#059669"]].map(([l,v,c])=>(<div key={l} style={{background:"#f8f9fa",borderRadius:8,padding:"10px 14px",flex:"1 1 80px",textAlign:"center"}}><div style={{fontSize:10,color:"#888"}}>{l}</div><div style={{fontSize:22,fontWeight:700,color:c}}>{v}</div></div>))}
        </div>}
        <div style={{display:"flex",gap:8,marginBottom:12}}>
          <button onClick={dlCSV} style={{...bn(true),background:"#059669",flex:1}}>CSV Indir</button>
          <button onClick={clearAllData} style={{...bn(true),background:"#dc2626",flex:1}}>Tüm Verileri Sil</button>
        </div>
        <div style={{maxHeight:600,overflowY:"auto",fontSize:11}}>
          {adminData.map((r,i)=>{const ex=expRow===i;return(<div key={i} style={{border:"1px solid #e5e7eb",borderRadius:8,marginBottom:8,overflow:"hidden"}}>
            ...
            {ex&&<div style={{padding:"12px 16px",background:"#fff",fontSize:11}}>
              <div style={{marginBottom:8,padding:"6px 10px",background:"#f8f9fa",borderRadius:6,fontSize:10}}>
                <b>Cinsiyet:</b> {r.demographics?.gender} | <b>Doğum:</b> {r.demographics?.birth_year}
                {r.bonus&&<span> | <b>Ders:</b> {r.bonus.course} | <b>No:</b> {r.bonus.student_no}</span>}
              </div>
              {(r.cycles||[]).map((c,ci)=>(<div key={ci} style={{marginBottom:16,padding:"10px 12px",background:ci%2?"#fafafa":"#fff",borderRadius:8,border:"1px solid #eee"}}>
                ...
                {/* Winning hypothesis */}
                <div style={{marginBottom:6,padding:"6px 10px",background:"#f0fdf4",borderRadius:6}}>
                  <div style={{fontWeight:700,fontSize:9,color:"#059669"}}>Hayatta Kalan Hipotez</div>
                ...
                {/* Revised hypothesis */}
                {c.revised_hypothesis?.needs_revision&&<div style={{marginBottom:6,padding:"6px 10px",background:"#fff7ed",borderRadius:6,border:"1px solid #fed7aa"}}>
                  <div style={{fontWeight:700,fontSize:9,color:"#ea580c"}}>Hipotez Güncellendi</div>
                  <div style={{fontSize:9,color:"#888",marginTop:2}}><b>Eski:</b> {c.revised_hypothesis.original_hypothesis?.label} — {c.revised_hypothesis.original_hypothesis?.description}</div>
                  <div style={{fontSize:10,marginTop:2}}><b>Yeni:</b> {c.revised_hypothesis.revised_hypothesis?.label} — {c.revised_hypothesis.revised_hypothesis?.description}</div>
                  <div style={{fontSize:9,color:"#666",marginTop:4,fontStyle:"italic"}}>{c.revised_hypothesis.revision_reasoning_tr}</div>
                  {c.revised_hypothesis.misprediction_analysis?.map((m,mi)=>(
                    <div key={mi} style={{fontSize:9,marginTop:4,padding:"4px 6px",background:"#fef3e2",borderRadius:4}}>
                      <b>{m.test_id}:</b> Tahmin {m.predicted} → Gerçek {m.actual}<br/>
                      <span style={{color:"#888"}}>Katılımcı: "{m.participant_explanation?.slice(0,80)}"</span><br/>
                      <span style={{color:"#666"}}>{m.analysis_tr}</span>
                    </div>))}
                </div>}
                {/* Eliminated */}
                {(c.elimination_log||[]).length>0&&<div style={{marginBottom:6,padding:"6px 10px",background:"#fef2f2",borderRadius:6}}>
                  <div style={{fontWeight:700,fontSize:9,color:"#dc2626"}}>Elenen Hipotezler ({(c.elimination_log||[]).length})</div>
                  {(c.elimination_log||[]).map((el,ei)=><div key={ei} style={{fontSize:9,marginBottom:2}}><b>{el.id}</b> [{el.label}] R{el.round||el.eliminated_at_round} — {el.verdict||el.kill_shot}: {el.evidence?.slice(0,60)}</div>)}
                </div>}
                {/* Profiling */}
                <div style={{marginBottom:6}}>
                  <div style={{fontWeight:700,fontSize:9,color:"#6366f1"}}>Profilleme ({c.profiling_rounds} soru)</div>
                  {c.profiling_history?.map((ph,pi)=><div key={pi} style={{fontSize:9,padding:"2px 0",borderBottom:"1px solid #f0f0f0"}}>
                    <b>S{ph.round}:</b> {ph.scenario?.slice(0,80)}... → <b>{ph.answer}</b>
                    {ph.reason&&<span style={{color:"#888"}}> — "{ph.reason.slice(0,60)}..."</span>}
                  </div>)}
                </div>
                {/* Test */}
                <div>
                  <div style={{fontWeight:700,fontSize:9,color:"#1a1a2e"}}>Test Soruları</div>
                  {c.test_questions?.map((tq,ti)=><div key={ti} style={{fontSize:9,padding:"3px 0",borderBottom:"1px solid #f0f0f0"}}>
                    <div>{tq.scenario?.slice(0,70)}...</div>
                    <div>Tahmin:<b>{tq.prediction}</b> ({tq.confidence}) Gerçek:<b>{tq.actual}</b> <b style={{color:tq.correct?"#059669":"#dc2626"}}>{tq.correct?"✓":"✗"}</b></div>
                    <div style={{color:"#888",fontSize:8}}>Gerekçe: {tq.prediction_reasoning_tr?.slice(0,80)}</div>
                    {tq.misprediction_feedback&&<div style={{color:"#ea580c",fontStyle:"italic",marginTop:2}}>Katılımcı açıklaması: "{tq.misprediction_feedback.slice(0,100)}"</div>}
                  </div>)}
                </div>
              </div>))}
            </div>}
  ...
  if(phase==="admin_login")return(<div style={pg}><Head><title>Admin</title></Head><div style={cd}>
    ...
    <input type="password" placeholder="Şifre" value={adminPw} onChange={e=>setAdminPw(e.target.value)} onKeyDown={e=>{if(e.key==="Enter"&&adminPw===ADMIN_PASS)setPhase("admin");}} style={inp}/>
  ...
  if(phase==="welcome")return(<div style={pg}><Head><title>CogniTwin</title></Head><div style={cd}>
    ...
    <p style={{fontSize:14,color:"#555",lineHeight:1.6,margin:"0 0 8px"}}>Bu çalışmada size 5 farklı konuda karar senaryoları sunulacak. Her senaryoda iki seçenekten birini tercih etmenizi ve nedenini kısaca açıklamanızı istiyoruz.</p>
    ...
    <p style={{fontSize:13,color:"#999",margin:"0 0 16px"}}>Tahmini süre: 25-40 dakika</p>
    <button onClick={()=>setPhase("demographics")} style={bn(true)}>Başla</button>
  ...
  if(phase==="demographics"){
    const canGo=gender&&birthYear.length===4&&parseInt(birthYear)>=1940&&parseInt(birthYear)<=2010;
    return(<div style={pg}><Head><title>CogniTwin</title></Head>{bar}<div style={cd}>
      <h2 style={{fontSize:16,fontWeight:600,margin:"0 0 16px"}}>Demografik Bilgiler</h2>
      <div style={{marginBottom:16}}>
        <label style={{fontSize:14,fontWeight:500,display:"block",marginBottom:8}}>Cinsiyetiniz:</label>
        <button onClick={()=>setGender("Kadın")} style={op(gender==="Kadın")}>Kadın</button>
        <button onClick={()=>setGender("Erkek")} style={op(gender==="Erkek")}>Erkek</button>
        <button onClick={()=>setGender("Diğer/Belirtmek istemiyorum")} style={op(gender==="Diğer/Belirtmek istemiyorum")}>Diğer / Belirtmek istemiyorum</button>
      </div>
      <div style={{marginBottom:8}}>
        <label style={{fontSize:14,fontWeight:500,display:"block",marginBottom:8}}>Doğum yılınız:</label>
        <input type="number" placeholder="Örn: 1998" value={birthYear} onChange={e=>setBirthYear(e.target.value)} style={inp} min="1940" max="2010"/>
      </div>
      <button onClick={startCycle} disabled={!canGo} style={bn(canGo)}>Devam</button>
    </div></div>);}
  ...
  if(phase==="error")return(<div style={pg}><Head><title>CogniTwin</title></Head><div style={cd}>
    ...
    <button onClick={()=>{setPhase("welcome");resetCycle();setCycle(1);setCycleResults([]);}} style={bn(true)}>Başa Dön</button>
  </div></div>);

  if(phase==="profiling"&&currentQ){
    const alive=hypotheses.filter(h=>h.status==="alive").length;
    return(<div style={pg}><Head><title>Konu {cycle} - Soru {profRound}</title></Head>{bar}<div style={cd}>
  ...
  if(phase==="test"&&testQs.length>0&&testIdx<testQs.length){
    const tq=testQs[testIdx];
    // Feedback screen (wrong prediction)
    if(showFeedback&&currentFeedbackQ){
      const predOpt=currentFeedbackQ.prediction==="A"?currentFeedbackQ.option_a:currentFeedbackQ.option_b;
      const predReason=currentFeedbackQ.prediction_reasoning_tr||"";
      return(<div style={pg}><Head><title>Konu {cycle} - Geri Bildirim</title></Head>{bar}<div style={cd}>
        <div style={{padding:"14px 16px",background:"#fef2f2",borderRadius:8,marginBottom:16,border:"1px solid #fecaca"}}>
          <p style={{fontSize:13,fontWeight:600,color:"#dc2626",margin:"0 0 8px"}}>Tahminimiz tutmadı</p>
          <p style={{fontSize:13,color:"#555",lineHeight:1.6,margin:"0 0 6px"}}>Önceki cevaplarınıza dayanarak bu soruda <b>"{predOpt}"</b> seçeneğini tercih edeceğinizi düşünmüştük.</p>
          <p style={{fontSize:12,color:"#777",lineHeight:1.5,margin:0,fontStyle:"italic"}}>Gerekçemiz: {predReason}</p>
        </div>
        <label style={{fontSize:14,fontWeight:500,display:"block",marginBottom:4}}>Tahminimiz neden yanlış çıktı? Bu tercihi yapma nedeninizi açıklar mısınız?</label>
    ...
    return(<div style={pg}><Head><title>Konu {cycle} - Test {testIdx+1}</title></Head>{bar}<div style={cd}>
      ...
      <button onClick={submitTestAnswer} disabled={!testAns[testIdx]} style={bn(!!testAns[testIdx])}>{testIdx===2?(cycle<TOTAL_CYCLES?"Konuyu Tamamla":"Son Adıma Geç"):"Sonraki"}</button>
    </div></div>);}

  if(phase==="bonus_info")return(<div style={pg}><Head><title>CogniTwin</title></Head>{bar}<div style={cd}>
    <h2 style={{fontSize:16,fontWeight:600,margin:"0 0 16px"}}>Son Bir Adım</h2>
    <p style={{fontSize:14,color:"#555",lineHeight:1.6,margin:"0 0 16px"}}>Bu çalışmaya bir ders kapsamında bonus puan karşılığı mı katılıyorsunuz?</p>
    <button onClick={()=>setIsBonus(true)} style={op(isBonus===true)}>Evet — bonus puan için katılıyorum</button>
    <button onClick={()=>setIsBonus(false)} style={op(isBonus===false)}>Hayır — gönüllü katılıyorum</button>
    {isBonus===true&&<div style={{marginTop:16}}>
      <label style={{fontSize:14,fontWeight:500,display:"block",marginBottom:8}}>Hangi ders için katılıyorsunuz?</label>
      <input type="text" placeholder="Ders adı" value={bonusCourse} onChange={e=>setBonusCourse(e.target.value)} style={{...inp,marginBottom:12}}/>
      <label style={{fontSize:14,fontWeight:500,display:"block",marginBottom:8}}>Öğrenci numaranız:</label>
      <input type="text" placeholder="Öğrenci numarası" value={studentNo} onChange={e=>setStudentNo(e.target.value)} style={inp}/>
    </div>}
    <button onClick={saveAll} disabled={isBonus===null||(isBonus&&(!bonusCourse||!studentNo))} style={bn(isBonus===false||(isBonus&&bonusCourse&&studentNo))}>Anketi Tamamla</button>
  </div></div>);

  if(phase==="done")return(<div style={pg}><Head><title>CogniTwin</title></Head><div style={{...cd,textAlign:"center",padding:"40px 20px"}}>
    <div style={{fontSize:48,marginBottom:12}}>&#10004;&#65039;</div>
    <h2 style={{fontSize:20,fontWeight:700,margin:"0 0 8px"}}>Teşekkürler!</h2>
    <p style={{fontSize:14,color:"#555",margin:"0 0 8px"}}>Katılımınız başarıyla kaydedildi.</p>
    <p style={{fontSize:12,color:"#999"}}>Katılımcı no: {pid}</p>
  </div></div>);
